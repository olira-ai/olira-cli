...
```bash
olira login
olira keys create --name "my-agent" --scopes sdk:historical-ingest api:manage-patients sdk:state-read sdk:integrations sdk:actions
```
...
query commands, key management, or outbound-action destinations and
deliveries) so a task only loads what it needs.
